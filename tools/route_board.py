...
* In1.Cu is a dense routed GND mesh with explicit clearance around every
  non-ground through connection.  It is emitted as copper tracks, not an
  unfilled KiCad zone, so the manufacturing image is deterministic even in a
...
# Short In1 ties for the few raster-plane fragments that are enclosed between
# nearby signal-via clearance holes.  Each endpoint is an existing GND via;
# exact copper clearance is checked independently after generation.
GROUND_REVIEWED_BRIDGES = [
    ((95.500, 16.875), (96.750, 13.875)),
    ((105.500, 16.750), (105.750, 6.000)),
    ((79.250, 29.250), (77.700, 36.700)),
    ((129.625, 37.000), (130.000, 51.875)),
    ((100.000, 29.750), (101.500, 33.625)),
]
...
def remove_reviewed_redundant_vias(board):
    """Remove reviewed route transitions that ended up staying on F.Cu.

    The deterministic router initially reserved these transitions while
    solving the surrounding nets, but the final SYS_5V and PN_MOSI paths both
    reach all endpoints entirely on F.Cu.  A through-via at either location is
    therefore electrically inert and KiCad correctly reports it as dangling.
    """
    reviewed = {
        ("SYS_5V", 89.000, 22.750),
        ("PN_MOSI", 89.500, 28.625),
        # With TP3 moved clear of C11, PN_MISO_IC can complete on F.Cu and
        # leaves this provisional transition unused.
        ("PN_MISO_IC", 87.000, 25.375),
    }
    removed = 0
    kept = []
    for item in board.traceItems:
        if isinstance(item, Via):
            key = (
                board.nets[item.net].name,
                round(item.position.X, 3),
                round(item.position.Y, 3),
            )
            if key in reviewed:
                removed += 1
                continue
        kept.append(item)
    # Placement changes can cause the router to avoid one of these provisional
    # transitions entirely.  Absence is already the desired state; any new
    # dangling transition is caught by the official KiCad DRC gate.
...
def add_ground_connections(
    ...
    *, fanout_only=False, ground_vias=None,
    ...
    for start, end in GROUND_REVIEWED_BRIDGES:
        segment(board, net_by_name, "GND", start, end, 0.35, "In1.Cu")
...
def apply_reviewed_manufacturing_neckdowns(board):
    """Apply local width relief without changing the proven maze topology."""
    matched = 0
    for item in board.traceItems:
        if not isinstance(item, Segment) or item.layer != "F.Cu":
            continue
        if board.nets[item.net].name != "3V3":
            continue
        vertical = abs(item.start.X - 83.0) < 1e-6 and abs(item.end.X - 83.0) < 1e-6
        y_values = sorted((item.start.Y, item.end.Y))
        reviewed_vertical = (
            vertical
            and abs(y_values[0] - 33.875) < 1e-6
            and abs(y_values[1] - 36.250) < 1e-6
        )
        reviewed_corner = (
            abs(item.start.Y - 33.875) < 1e-6
            and abs(item.end.Y - 33.875) < 1e-6
            and sorted((item.start.X, item.end.X)) == [82.875, 83.0]
        )
        if reviewed_vertical or reviewed_corner:
            # This short U5 supply approach carries only logic current.  The
            # router reserved a 0.50 mm corridor, but 0.40 mm manufactured
            # copper restores >=0.10 mm clearance to the SYS_5V fan-out.
            item.width = 0.40
            matched += 1
    if matched != 2:
        raise RuntimeError(
            f"Expected two reviewed 3V3 neck-down segments, found {matched}"
        )
...
def main() -> None:
    ...
    removed_dangling_vias = remove_reviewed_redundant_vias(board)
    ...
    segment_count = sum(isinstance(item, Segment) for item in board.traceItems)
    ...
    lines = [
        ...
        f"Reviewed single-layer route vias removed: {removed_dangling_vias}",
    ...
    lines.extend(
        [
            "GND CONNECTIVITY: explicit dogbone/thermal vias to deterministic In1 mesh",
