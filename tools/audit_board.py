...
def main():
    ...
    # The ground pours ship unfilled, so there is no filled polygon to test.
    # Test instead what the filler is allowed to do: the pour outline minus the
    # copper-pour keep-outs.  A plane under either antenna is a shorted turn,
    # and this is the check that keeps the keep-out rectangles honest rather
    # than merely present.
    keepout_shapes = defaultdict(list)
    for zone in board.zones:
        settings = zone.keepoutSettings
        if settings is not None and settings.copperpour == "not_allowed":
            for layer in zone.layers:
                keepout_shapes[layer].append(zone_outline(zone))
    for zone in board.zones:
        if zone.keepoutSettings is not None:
            continue
        for layer in zone.layers:
            fillable = zone_outline(zone)
            if keepout_shapes[layer]:
                fillable = fillable.difference(unary_union(keepout_shapes[layer]))
            for name, region in (("NFC", nfc_keepout), ("ESP ANTENNA", esp_keepout)):
                # Area, not is_empty: the keep-out rectangles share edges with
                # the audited ones, and a shared edge intersects as a
                # zero-area line.
                overlap = fillable.intersection(region)
                if overlap.area > 1e-6:
                    errors.append(
                        f"{name} KEEP-OUT VIOLATION: {layer} zone '{zone.name}' "
                        f"may fill {overlap.area:.2f} mm^2 inside the keep-out"
                    )
