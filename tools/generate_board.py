...
FIXED_PLACEMENT = {
    ...
    "C1": (114.0, 68.0, 90), "C2": (103.2, 54.7, 0),
...
def apply_reference(footprint: Footprint, reference: str, value: str) -> None:
    ...
    # Footprints written by different KiCad generations store the reference
    # either as an fp_text graphic item or as a (property "Reference").  Set
    # whichever this footprint uses so the chain does not depend on the library
    # revision it was parsed from.
    if getattr(footprint, "properties", None):
        if "Reference" in footprint.properties:
            footprint.properties["Reference"] = reference
        if "Value" in footprint.properties:
            footprint.properties["Value"] = value
...
def create_mounting_hole(reference: str, x: float, y: float) -> Footprint:
    source = LIB_OUT / "MountingHole_3.2mm_M3.kicad_mod"
...
def generate() -> None:
    ...
    # Seed the project library from upstream only when it has no copy yet.
    # Overwriting unconditionally binds the project to whichever KiCad library
    # is installed: KiCad 10's footprints store the reference as
    # (property "Reference"), which apply_reference() does not recognise, so a
    # rebuild would silently emit REF** for the mounting holes.
    mounting_target = LIB_OUT / "MountingHole_3.2mm_M3.kicad_mod"
    if not mounting_target.is_file():
        mounting_source = (
            WORKSPACE_ROOT / ".tools" / "kicad-root" / "usr" / "share" / "kicad" /
            "footprints" / "MountingHole.pretty" / "MountingHole_3.2mm_M3.kicad_mod"
        )
        mounting_target.write_text(
    ...
    for part in parts:
        if part.ref == "AE1":
            ...
            # The project library first.  copy_libraries() exists to "make the
            # KiCad project self-contained and relocatable", and this library
            # already holds every footprint the board uses at the revision the
            # released board was built from.  Reaching upstream instead binds
            # the chain to an installed KiCad library, and the staging trees
            # (tmp/easyeda, tmp/vendor) are not shipped at all.
            source = LIB_OUT / f"{part.footprint.split(':', 1)[1]}.kicad_mod"
            if not source.is_file():
                try:
                    ...
                    raise FileNotFoundError(
                        f"No footprint source for {part.ref}: not in the project "
                        f"library and no upstream mapping ({exc})"
                    ) from exc
        footprint = Footprint.from_file(str(source))
        if part.ref == "U4":
